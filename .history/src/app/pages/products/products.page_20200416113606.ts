...
export class ProductsPage implements OnInit {
  ...
  newProduct(product: Product) {
    const i = this.sections.findIndex(p => p.name === product.section);
    if (i >= 0) {
      if (!this.sections[i].products || this.sections[i].products) this.sections[i].products = []
  ...
  saveColor(segment?) {
    ...
    if (segment) this.setSegementColors()
  }
  ...
  getProfile() {
    ...
    .then(profile => {
      ...
      this.themeService.setAppTheme(profile.colors)
      this.getSections()
    })
    .catch(err => {
      alert(err)
  ...
  setSegementColors() {   
    if (this.profile.colors.segmentButton) document.body.style.setProperty('--segmentButtonColor', this.profile.colors.segmentButton);
    else document.body.style.setProperty('--segmentButtonColor', 'black');

    if (this.profile.colors.segmentButtonFocused) document.body.style.setProperty('--segmentButtonFocusedColor', this.profile.colors.segmentButtonFocused);
    else document.body.style.setProperty('--segmentButtonFocusedColor', 'blue');

    if (this.profile.colors.backgroundCard) document.body.style.setProperty('--bacground-card', this.profile.colors.backgroundCard);
    else document.body.style.setProperty('--bacground-card', 'white');
  }
